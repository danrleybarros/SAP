...
{
    public class FatFileGeneratorTest : IClassFixture<Fixture.ApplicationFixture>
    {
        private SAP.Application.Repositories.IFileGenerator<FATFaturado> fatFileGeneration;

        private const string headerFormat = @"(IDFAT55)(\d\d\d\d\d\d\d\d\d\d)(19|20)\d\d(0[1-9]|1[012])(0[1-9]|[12][0-9]|3[01])\.TXT   (0[1-9]|[12][0-9]|3[01])(0[1-9]|1[012])(19|20)\d\d([0-5]?\d)([0-5]?\d)([0-5]?\d)";
        private const string valueFormat = @"(D1)(\d{10})(0[1-9]|[12][0-9]|3[01])(0[1-9]|1[012])(19|20)\d{2}(FAT)(\s{7})((\w|\s){15})(\s{2})(\s*)(\d+\,\d{2})(\s{15}\s{16}(\s{7}))(GW29TR018233)((\w|\s){12})(\s{12})(01)(19|20)\d\d(0[1-9]|1[012])(0[1-9]|[12][0-9]|3[01])(\s{10}\s{16})(\d{4})([a-zA-Z\u00C0-\u00FF ]{20})(Pos|Pre)(\s{1})((D|C)\s{1})([a-zA-Z\u00C0-\u00FF ]{10})(\w{2})";
        private const string header2Format = @"(HH)(FAT)(TBRA)(29SP)";
        private const string footerFormat = @"(FF)(\d+).*?(\d+)(,)(\d\d)";

        public FatFileGeneratorTest(Fixture.ApplicationFixture fixture)
        {
            fatFileGeneration = fixture.Container.Resolve<SAP.Application.Repositories.IFileGenerator<FATFaturado>>();
        }
        ...
        {
            var model = Builders.Build.FAT.Build();
            var strFile = fatFileGeneration.Generate(model);
