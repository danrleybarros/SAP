...
{
    public class ARRBoletoFileGeneratorTest : IClassFixture<Fixture.ApplicationFixture>
    {
        private SAP.Application.Repositories.IFileGenerator<SAP.Domain.ARR.Boleto.ARRBoleto> ARRFileGeneration;

        private string identificationFormat = @"^IDARR56TB29\d{2}\d{4}.TXT (0[1-9]|[12][0-9]|3[01])(0[1-9]|1[012])(19|20)\d\d\d{6}";
        private string headerFormat = @"^HHARRTBRA\s{8}\s{8}(0[1-9]|1[012])(19|20)\d\d29SP";
        private string lineFormat = @"^D1(\d{10})((0[1-9]|[12][0-9]|3[01])(0[1-9]|1[012])(19|20)\d\d)BCO([0-9 ]{7})([a-zA-Z0-9\s]{15})(\s{2})([0-9 ]{13},\d{2})(\s{10})";
        private string footerFormat = @"^FF\d{6}\s{16}\s{16}\s*\d+,\d{2}";

        public ARRBoletoFileGeneratorTest(Fixture.ApplicationFixture fixture)
        {
            ARRFileGeneration = fixture.Container.Resolve<SAP.Application.Repositories.IFileGenerator<SAP.Domain.ARR.Boleto.ARRBoleto>>();
        }
        ...
        {
            var model = Builders.Build.ARRBoleto.Build();
