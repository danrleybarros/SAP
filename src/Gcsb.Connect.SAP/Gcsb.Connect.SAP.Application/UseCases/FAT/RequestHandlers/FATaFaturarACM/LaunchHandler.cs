...
namespace Gcsb.Connect.SAP.Application.UseCases.FAT.RequestHandlers.FATaFaturarACM
{
    public class LaunchHandler : Handler
    {
        ...
        public override void ProcessRequest(FATRequest request)
        {
            ...
            if (!error && DeferralAccountingEntry.IsValidFinancialAccount)
            {
                ...
                stores.ForEach(store =>
                {
                    ...
                    var maxDate = Convert.ToInt32(Environment.GetEnvironmentVariable("BILL_MAX_DATE"));
                    var qtdDays = 30 - (maxDate == 31 ? 30 : maxDate);
        ...
        private List<Domain.FAT.FATaFaturarACM.LaunchACM> GetLines(
            ...
            Domain.FinancialAccountsClient.FinancialAccount.AccountDetailsByServiceDto accountDetailsByServiceDto = null)
        {
            return services
               ...
               .GroupBy(g => new { g.a.FinancialAccount, g.s.Invoice.Customer.BillingStateOrProvince, g.s.Invoice.PaymentMethod, g.s.ServiceType, g.a.AccountingEntryType, g.a.HaveIntercompany })
               .Where(w => Convert.ToDecimal(w.Sum(sum => sum.s.GrandTotalRetailPrice)) + Convert.ToDecimal(w.Sum(sum => sum.s.TotalDiscount)) > 0M)
               .Select((s, index) => new Domain.FAT.FATaFaturarACM.LaunchACM(
                   (index + 1),
                   cycle.AddMonths(1),
                   s.Key.FinancialAccount,                   
                   Math.Round(((Convert.ToDecimal(s.Sum(sum => sum.s.GrandTotalRetailPrice)) + Convert.ToDecimal(s.Sum(sum => sum.s.TotalDiscount))) / 30)  * qtdDays, 2),
