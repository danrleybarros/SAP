...
{
    public class GenerateFileHandler : Handler<ARRBoleto>, IGenerateFileHandler<ARRBoleto>
    {
        private readonly IFileGenerator<ARRBoleto> fileGenerator;
        private readonly string path;

        public GenerateFileHandler(IFileGenerator<ARRBoleto> fileGenerator)
        {
        ...
        {
            request.AddProcessingLog("Generatin ARR Boleto");

            request.ARRDomain = new List<ARRBoleto>();
            ...
            request.Lines.ToList().ForEach(f =>
            {
                ...
                var arr = new ARRBoleto(identification, new Header(f.Key), f.Value, file);
